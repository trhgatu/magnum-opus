...
for (const requiredDocument of requiredProductDocuments) {
  if (!existsSync(path.join(repositoryRoot, requiredDocument))) {
    errors.push(`${requiredDocument}: required product document is missing`);
  }
}
...
console.log(
  `Product documentation verified: ${requiredProductDocuments.length} required files, ${markdownFiles.length} Markdown files, no broken local links.`,
);
