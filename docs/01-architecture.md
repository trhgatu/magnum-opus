...
| `forge`         | Habit (BUILD và QUIT-type), Habit Check-in và Habit Relapse đã chạy full-stack; Routine đã có backend V1; Routine client và trang Today chưa được triển khai. |
...
`analytics/dashboard`, `reflection/journal`, `forge/habit`, `forge/habit-check-in`, `forge/habit-relapse` và `forge/routine` là submodule bên trong context cha. Context cha chỉ gom module con; nó không chứa business logic trá hình. `HabitModule`, `HabitCheckInModule`, `HabitRelapseModule` và `RoutineModule` đều có controller, handlers và adapter nên được `ForgeModule` nối vào `AppModule`.
