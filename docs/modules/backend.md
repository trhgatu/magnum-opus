...
### Forge / Habit, Habit Check-in, Habit Relapse và Routine runtime

Forge được đăng ký vào API qua `ForgeModule`, hiện gom `HabitModule`, `HabitCheckInModule`, `HabitRelapseModule` và `RoutineModule`. Habit có `type: 'BUILD' | 'QUIT'`, cố định lúc tạo và bất biến sau đó. Habit có các endpoint `POST /habits`, `GET /habits` (hỗ trợ filter `type`), `GET /habits/:id`, `PUT /habits/:id`, `PATCH /habits/:id/archive` và `PATCH /habits/:id/restore` — `frequency` chỉ áp dụng cho BUILD-type, `quitStartedAt` chỉ áp dụng cho QUIT-type. Check-in có `PUT /habits/:habitId/check-ins/today`, `DELETE /habits/:habitId/check-ins/today`, `GET /habits/:habitId/check-ins/today` và `GET /habits/:habitId/check-ins?from=...&to=...` — chỉ áp dụng cho Habit BUILD-type, gọi trên Habit QUIT-type bị từ chối. Endpoint đọc `today` trả cả calendar date theo `User.timeZone` và record hiện tại để client không phải suy đoán ngày nghiệp vụ. Tất cả đều đi qua `JwtAuthGuard`, lấy owner từ access token và không nhận owner trong payload.
...
Habit Relapse là bounded context riêng cho QUIT-type, có `POST /habits/:habitId/relapses` (ghi 1 bản ghi mới, append-only, không idempotent theo ngày — khác hẳn Check-in) và `GET /habits/:habitId/progress` (trả `since`/`sinceReason`/`daysSince`, tính từ bản ghi relapse gần nhất có `occurredAt >= quitStartedAt` hiện tại, hoặc từ `quitStartedAt` nếu không có relapse nào thỏa). Hai endpoint không có cùng guard: `LogRelapseHandler` chỉ chấp nhận Habit đang `ACTIVE` **và** thuộc loại QUIT (gọi trên BUILD-type hoặc Habit archived trả `409`); `GetHabitProgressHandler` chỉ kiểm tra `type === 'QUIT'`, không kiểm tra `isActive` — xem progress cho một Habit QUIT đã archived vẫn trả `200` với dữ liệu bình thường. "daysSince" tính theo ngày lịch (calendar day) theo timezone của owner, nhất quán cách Today context đã làm — không dùng UTC hay giờ server.
...
Routine aggregate chỉ giữ ordered Habit IDs. Nó không import Habit domain. Khi thêm membership, application đọc `{ id, isActive, type }` qua `RoutineHabitReader`, một consumer-owned port, để kiểm tra Habit cùng owner, còn active và thuộc loại BUILD — thêm Habit QUIT-type vào Routine bị `AddRoutineHabitHandler` từ chối (`HabitTypeNotAllowedInRoutineException`), cùng cách guard `isActive` hiện có, không phải check bên trong aggregate `Routine`. `PrismaRoutineRepository` compare-and-swap Routine trước rồi mới thay membership trong cùng transaction. Nếu revision đã stale, transaction dừng mà không xóa hoặc ghi lại thứ tự cũ.
...
`HabitCheckIn` đã có E2E cho idempotency, ownership, archive guard, history và undo. `HabitRelapse` chưa có PostgreSQL E2E riêng (`apps/server/test/*.e2e-spec.ts`) — coverage hiện dừng ở unit/spec test cho domain, handler, mapper, reader, repository, presenter và controller. Habit (cả hai loại), Habit Check-in, Habit Relapse, Routine và trang Today đều đã có client.
