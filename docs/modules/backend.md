...
Habit Relapse là bounded context riêng cho QUIT-type, có `POST /habits/:habitId/relapses` (ghi 1 bản ghi mới, append-only, không idempotent theo ngày — khác hẳn Check-in) và `GET /habits/:habitId/progress` (trả `since`/`sinceReason`/`daysSince`, tính từ bản ghi relapse gần nhất có `occurredAt >= quitStartedAt` hiện tại, hoặc từ `quitStartedAt` nếu không có relapse nào thỏa). Cả hai endpoint chỉ chấp nhận Habit đang `ACTIVE` và thuộc loại QUIT; gọi trên BUILD-type hoặc Habit archived trả `409`. "daysSince" tính theo ngày lịch (calendar day) theo timezone của owner, nhất quán cách Today context đã làm — không dùng UTC hay giờ server.
...
`HabitCheckIn` đã có E2E cho idempotency, ownership, archive guard, history và undo. `HabitRelapse` có E2E cho append-only logging, type/active guard và progress calculation. Habit (cả hai loại), Habit Check-in và Habit Relapse đã có client; Routine hiện mới hoàn tất backend V1. Routine client và trang Today vẫn chưa được triển khai.
